...
import { IconFromClass } from 'src/functions';
...
function Window(client) {
    ...
    return Widget.EventBox({
        child: Widget.Button({
            on_clicked: () => Hyprland.sendMessage(`dispatch focuswindow title:${client.title}`),
            child: Widget.Box({
                class_name: `client-box`,
                children: [
                    Widget.Icon({
                        setup: self => IconFromClass(client.initialClass).then(icon => self.icon = icon),
                        size: 20
                    }),
                    Widget.Revealer({
                        reveal_child: Hyprland.active.client.bind('address')
        ...
        class_name: Hyprland.active.client.bind('address')
            .transform(address => address == client.address ? 'focused' : '' )
        }),
        on_hover: self => self.child.child.children[1].reveal_child = true,
        on_hover_lost: self => {if (Hyprland.active.client.address != client.address) self.child.child.children[1].reveal_child = false},
    })
}
...
function workspaces() {
    return Hyprland.workspaces.sort((a,b) => a.id - b.id).map(({id}) => Widget.Box({
